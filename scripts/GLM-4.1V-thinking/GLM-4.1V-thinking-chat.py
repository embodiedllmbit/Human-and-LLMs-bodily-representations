...
GLM_FULLFLOW_MAX_NEW_TOKENS = int(os.environ.get("EMBODIED_GLM_FULLFLOW_MAX_NEW_TOKENS", "4096"))
GLM_FULLFLOW_RETRY_MAX_NEW_TOKENS = int(os.environ.get("EMBODIED_GLM_FULLFLOW_RETRY_MAX_NEW_TOKENS", "4096"))
GLM_FULLFLOW_REPETITION_PENALTY = float(os.environ.get("EMBODIED_GLM_FULLFLOW_REPETITION_PENALTY", "1.03"))
GLM_FULLFLOW_SYSTEM_PROMPT = (
...
class GLM4VThinkingInference:
    ...
    def _normalize_first_fullflow_response(self, text: str) -> str:
        if not text:
            return ""
    ...
    def _build_effective_prompt(self, prompt_text):
        return prompt_text + FULLFLOW_GUIDANCE

    def _build_retry_prompt(self, prompt_text):
        return self._build_effective_prompt(prompt_text) + FULLFLOW_RETRY_GUIDANCE

    def _build_messages(self, video_path, prompt_text):
        return [
            {
                "role": "system",
                "content": [{"type": "text", "text": GLM_FULLFLOW_SYSTEM_PROMPT}],
            },
            {
                "role": "user",
                "content": [
    ...
    def _generate_candidate(self, video_path, prompt_text, max_new_tokens):
        ...
        with torch.no_grad():
            ...
            generate_kwargs = {
                ...
                "do_sample": False,
                "repetition_penalty": GLM_FULLFLOW_REPETITION_PENALTY,
            }
        ...
        original_output = self._normalize_first_fullflow_response(raw_generated_output)
    ...
    def run_inference(self, video_path, prompt_text, prompt_type, video_id):
        ...
        try:
            self.captured_embeddings = {}
            retry_output = None
            max_new_tokens = min(DEFAULT_MAX_NEW_TOKENS_THINK, GLM_FULLFLOW_MAX_NEW_TOKENS)
            ...
            if initial_candidate['answer_payload']['final_answer'] is None:
                retry_candidate = self._generate_candidate(
                    ...
                    min(DEFAULT_MAX_NEW_TOKENS_THINK, GLM_FULLFLOW_RETRY_MAX_NEW_TOKENS),
                )
