...
INTERNVL_MAX_NEW_TOKENS_BASE = int(os.environ.get("EMBODIED_INTERNVL_FULLFLOW_MAX_NEW_TOKENS_BASE", "512"))
INTERNVL_MAX_NEW_TOKENS_THINK = int(os.environ.get("EMBODIED_INTERNVL_FULLFLOW_MAX_NEW_TOKENS_THINK", "4096"))
RETRY_MAX_NEW_TOKENS = int(os.environ.get("EMBODIED_INTERNVL_FULLFLOW_RETRY_MAX_NEW_TOKENS", "4096"))
...
FULLFLOW_REPETITION_PENALTY = float(
    os.environ.get("EMBODIED_INTERNVL_FULLFLOW_REPETITION_PENALTY", "1.0")
)
...
class InternVL35Inference:
    ...
    def _build_effective_question(self, question):
        return question + FULLFLOW_GUIDANCE

    def _build_retry_question(self, question):
        return self._build_effective_question(question) + FULLFLOW_RETRY_GUIDANCE
    ...
    def _run_fullflow_retry(self, pixel_values, num_patches_list, question, enable_thinking):
        ...
        if FULLFLOW_REPETITION_PENALTY != 1.0:
            retry_config['repetition_penalty'] = FULLFLOW_REPETITION_PENALTY
        with torch.no_grad():
    ...
    def run_inference(self, video_path, prompt_text, prompt_type, video_id):
        ...
        try:
            ...
            if enable_thinking:
                ...
                if FULLFLOW_REPETITION_PENALTY != 1.0:
                    generation_config['repetition_penalty'] = FULLFLOW_REPETITION_PENALTY
            else:
            ...
            if initial_candidate['answer_payload']['final_answer'] is None:
                retry_candidate = self._run_fullflow_retry(
                    pixel_values, num_patches_list, question, enable_thinking
                )
                retry_output = retry_candidate['original_output']
                selected_candidate, _ = self._select_candidate(initial_candidate, retry_candidate)
...
def main():
    ...
    parser.add_argument(
        ...
        '-m',
        type=str,
        default=str(PROJECT_ROOT / 'InternVL3.5' / 'models' / 'InternVL3_5-8B'),
    )
