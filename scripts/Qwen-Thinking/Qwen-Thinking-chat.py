...
RETRY_MAX_NEW_TOKENS = int(os.environ.get("EMBODIED_QWEN_FULLFLOW_RETRY_MAX_NEW_TOKENS", "4096"))
QWEN_THINK_MAX_NEW_TOKENS = int(os.environ.get("EMBODIED_QWEN_FULLFLOW_MAX_NEW_TOKENS", "4096"))
...
class Qwen3VLThinkingInference:
    ...
    def _build_messages(self, video_path, prompt_text):
        return [
            {
                "role": "system",
                "content": [{"type": "text", "text": QWEN_FULLFLOW_SYSTEM_PROMPT}],
            },
            {
                "role": "user",
                "content": [
    ...
    def _parse_candidate(self, raw_response):
        ...
        cleaned_response = normalize_first_fullflow_response(raw_response)
    ...
    def _run_fullflow_retry(self, video_path, prompt_text):
    ...
    def run_inference(self, video_path, prompt_text, prompt_type, video_id):
        ...
        try:
            ...
            if initial_candidate['answer_payload']['final_answer'] is None:
                retry_candidate = self._run_fullflow_retry(video_path, prompt_text)
...
def main():
    ...
    parser.add_argument(
        ...
        '-m',
        type=str,
        default=str(PROJECT_ROOT / 'Qwen-Thinking' / 'Qwen3-VL-8B-Thinking'),
    )
